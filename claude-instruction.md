You need to fix the team system so that:

Team members inherit company settings from the team owner
Only team owners pay for subscriptions (team members join free)
Invited users skip the full onboarding process
Team members cannot access billing or company settings

CRITICAL: Do NOT modify any non-team related code. Do NOT touch invoice, expense, income, or any other business logic. ONLY modify team-specific flows.
STEP 1: Fix Settings Inheritance
File: src/contexts/SettingsContext.tsx
Find the loadUserSettings function. Modify it to check if the user is a team member and load the owner's settings instead:
typescript// Add this logic to load team owner's settings for team members
// 1. Check if user is part of a team
// 2. If they are NOT the owner, load the team owner's settings
// 3. If they ARE the owner or solo user, load their own settings
// Keep all existing notification preferences personal
STEP 2: Fix Onboarding for Invited Users
File: src/components/Auth/Register.tsx (or wherever registration happens)
Add logic to detect if a user is accepting an invitation:

Check if email exists in pending_invites table
If yes, skip to a simplified onboarding (just name/password)
Auto-join them to the team
Skip plan selection, company setup, and settings configuration

File: Create new src/components/Auth/TeamJoinFlow.tsx
Create a simplified flow for invited users:

Welcome message: "You're joining [Team Name]"
Only collect personal info
Auto-apply team settings
Direct to dashboard

STEP 3: Hide Billing from Team Members
File: src/components/Layout/Sidebar.tsx (or wherever navigation is)
Conditionally hide billing/subscription menu items:
typescript// Only show billing link if user is team owner or solo user
// Check userRole from DataContext
// Hide: Billing, Subscription, Upgrade buttons for team members
File: src/components/Settings/Settings.tsx
Split settings into two sections:

Company Settings (only visible to owner)
Personal Preferences (visible to all)

STEP 4: Update Team Management UI
File: src/components/Settings/TeamManagement.tsx
Add these informational elements:

Show "Subscription: [Plan Name] - [X/Y] seats used"
For team members, show: "Account managed by [Owner Name]"
Disable company settings for non-owners

STEP 5: Fix Subscription Checks
File: src/contexts/DataContext.tsx
Ensure subscription is ALWAYS checked at team level:
typescript// You already have getEffectiveUserId working correctly
// Just ensure ALL subscription checks use effectiveUserId
// Don't create new subscription entries for team members
STEP 6: Add Guards to Prevent Team Member Access
Create a new component: src/components/Auth/OwnerOnlyRoute.tsx
typescript// Wrapper component that redirects team members to dashboard
// Use for: Billing pages, Subscription pages, Company settings
Wrap these routes with OwnerOnlyRoute:

/billing
/subscription
/settings/company

STEP 7: Update User Context Display
File: src/components/Layout/Header.tsx
Add team context indicator:
typescript// If user is team member, show: "Working in [Company Name]"
// Show role badge next to user name
MODIFICATIONS TO AVOID
DO NOT change:

Any invoice logic
Any expense/income logic
Any reporting logic
Database service files (except team-specific queries)
Category management
Client management
...
Solo user can still use app normally ✅
Team owner sees billing/settings ✅
Team member does NOT see billing ✅
Team member uses owner's currency/country ✅
Invited user skips plan selection ✅
...
src/components/Auth/[Registration files]
...
CREATE TABLE public.user_payment_accounts (
  ...
  CONSTRAINT user_payment_accounts_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.profiles(id)
);
