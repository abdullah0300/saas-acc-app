...
import { Search, Edit2, Save, X, Eye, AlertCircle, CheckCircle, AlertTriangle } from 'lucide-react';
import { useAuth } from '../../../contexts/AuthContext';
import { validateTitle, validateDescription, getSEOScoreLabel } from '../../../utils/seoValidation';
...
export const SEOManagerDashboard: React.FC = () => {
  ...
  const getCharacterCount = (text: string, optimal: number) => {
    ...
    const color = length < optimal * 0.8 ? 'text-yellow-600' : length > optimal * 1.2 ? 'text-red-600' : 'text-green-600';
    return { length, color };
  };
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">Manage SEO metadata for your public pages</p>
        </div>
          ...
          <div className="bg-white rounded-lg shadow-lg p-6">
            <div className="flex items-center justify-between mb-6">
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-600 mt-1">{editingPage.page_path}</p>
              </div>
              ...
              {(() => {
                ...
                return (
                  <div className={`flex items-center gap-2 px-4 py-2 rounded-lg border-2 ${
                    scoreData.color === 'green' ? 'bg-green-50 border-green-500' :
                    scoreData.color === 'blue' ? 'bg-blue-50 border-blue-500' :
                    scoreData.color === 'yellow' ? 'bg-yellow-50 border-yellow-500' :
                    scoreData.color === 'orange' ? 'bg-orange-50 border-orange-500' :
                    'bg-red-50 border-red-500'
                  }`}>
                    <span className="text-2xl">{scoreData.emoji}</span>
                    <div>
                      <div className="text-xs font-medium text-gray-600">SEO Score</div>
                      <div className="text-lg font-bold">{scoreData.score}/100</div>
                      <div className="text-xs text-gray-600">{scoreData.label}</div>
                    </div>
                  </div>
                );
              })()}
              ...
              <div className="border-b pb-6">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    />
                    <p className={`text-sm mt-1 ${getCharacterCount(editingPage.meta_title, 60).color}`}>
                      {getCharacterCount(editingPage.meta_title, 60).length} characters (Optimal: 50-60)
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    />
                    <p className={`text-sm mt-1 ${getCharacterCount(editingPage.meta_description, 160).color}`}>
                      {getCharacterCount(editingPage.meta_description, 160).length} characters (Optimal: 150-160)
                    </p>
                  </div>
              ...
              <div className="border-b pb-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <textarea
                      value={editingPage.og_description || editingPage.meta_description}
              ...
              <div>
                ...
                {showPreview && (
                  <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                    <p className="text-xs text-gray-500 mb-2">How it looks on Google:</p>
                    <div className="bg-white p-4 rounded">
                      <div className="text-blue-700 text-xl hover:underline cursor-pointer">
                        {editingPage.meta_title}
                      </div>
                      <div className="text-green-700 text-sm mt-1">
                        https://smartcfo.com{editingPage.page_path}
                      </div>
