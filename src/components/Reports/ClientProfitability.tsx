...
export const ClientProfitability: React.FC = () => {
  ...
  const [sortBy, setSortBy] = useState<"revenue" | "name" | "recent">(
    "revenue"
  );
  ...
  const loadReportData = async () => {
    ...
    try {
      ...
      const [clientsData, invoicesData, incomesData, creditNotesData] =
        await Promise.all([
          getClients(user.id),
          getInvoices(user.id),
  ...
  const processClientProfitability = (
    ...
    const profitData: ClientProfitData[] = clientsList.map((client) => {
      ...
      const clientIncomes = incomesList.filter(
        (inc) => inc.client_id === client.id
      );
      ...
      const clientCreditNotes = creditNotesList.filter(
        (cn: CreditNote) => cn.client_id === client.id && cn.applied_to_income
      );
      const totalCredits = clientCreditNotes.reduce(
        (sum: number, cn: CreditNote) => sum + (cn.base_amount || cn.total),
        0
      );
      ...
      const outstandingAmount = [...pendingInvoices, ...overdueInvoices].reduce(
        (sum, inv) => sum + (inv.base_amount || inv.total),
        0
      );
      ...
      const lastPayment = paidInvoices
        ...
        .sort(
          (a, b) =>
            parseISO(b.paid_date!).getTime() - parseISO(a.paid_date!).getTime()
        )[0];
      ...
      return {
        ...
        averageInvoiceValue:
          clientInvoices.length > 0 ? totalRevenue / clientInvoices.length : 0,
        outstandingAmount,
    ...
    const sortedData = profitData.sort(
  ...
  const calculateMonthlyRevenue = (incomes: Income[]) => {
    ...
    incomes.forEach((income) => {
      ...
      monthlyData[monthKey] =
        (monthlyData[monthKey] || 0) + (income.base_amount || income.amount);
    });
  ...
  const exportReport = () => {
    ...
    csv += "CLIENT DETAILS\n";
    csv +=
      "Client Name,Company,Email,Phone,Total Revenue,Invoices,Paid,Pending,Overdue,Outstanding,Avg Payment Days\n";
      ...
      case "recent":
        filtered.sort((a, b) => {
          const dateA = a.lastPaymentDate
            ? parseISO(a.lastPaymentDate).getTime()
            : 0;
          const dateB = b.lastPaymentDate
            ? parseISO(b.lastPaymentDate).getTime()
            : 0;
          return dateB - dateA;
        });
        break;
  ...
  const revenueDistributionData = top10Clients.map((client, index) => ({
    ...
    percentage: (
      (client.totalRevenue / summaryMetrics.totalRevenue) *
      100
    ).toFixed(1),
  }));
  ...
  const getClientTier = (revenue: number) => {
    if (revenue >= summaryMetrics.topClientRevenue * 0.8)
      return {
        tier: "Diamond",
        color:
          "text-purple-800 bg-gradient-to-r from-purple-100 to-indigo-200 border border-purple-300",
        icon: Crown,
      };
    if (revenue >= summaryMetrics.averageClientValue * 2)
      return {
        tier: "Gold",
        color:
          "text-amber-800 bg-gradient-to-r from-yellow-100 to-amber-200 border border-amber-300",
        icon: Award,
      };
    if (revenue >= summaryMetrics.averageClientValue)
      return {
        tier: "Silver",
        color:
          "text-gray-800 bg-gradient-to-r from-gray-100 to-gray-200 border border-gray-300",
        icon: Star,
      };
    return {
      tier: "Bronze",
      color:
        "text-orange-800 bg-gradient-to-r from-orange-100 to-red-200 border border-orange-300",
      icon: Target,
    };
  };
              ...
              <div className="flex items-start space-x-6">
                ...
                <div>
                  ...
                  <p className="text-lg text-gray-600 mb-4">
                    Deep insights into your client relationships and revenue
                    patterns
                  </p>
            ...
            <div className="flex space-x-3">
              ...
              <div>
                ...
                <select
                  value={sortBy}
                  onChange={(e) =>
                    setSortBy(e.target.value as "revenue" | "name" | "recent")
                  }
                  className="px-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-gray-50 hover:bg-white transition-all duration-200"
                >
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="text-right">
                  ...
                  <div className="text-xs text-blue-600 font-medium">
                    {summaryMetrics.activeClients} active
                  </div>
                </div>
              </div>
          ...
          <div className="bg-gradient-to-br from-emerald-50 to-green-100 rounded-2xl shadow-xl p-6 border border-emerald-200 relative overflow-hidden hover:shadow-2xl transition-all duration-300 group transform hover:scale-105">
            <div className="relative z-10">
              ...
              <p className="text-3xl font-bold text-emerald-900 mb-1">
                {formatCurrency(summaryMetrics.totalRevenue, baseCurrency)}
              </p>
          ...
          <div className="bg-gradient-to-br from-amber-50 to-orange-100 rounded-2xl shadow-xl p-6 border border-amber-200 relative overflow-hidden hover:shadow-2xl transition-all duration-300 group transform hover:scale-105">
            <div className="relative z-10">
              ...
              <p className="text-3xl font-bold text-amber-900 mb-1">
                {formatCurrency(summaryMetrics.topClientRevenue)}
              </p>
          ...
          <div className="bg-gradient-to-br from-red-50 to-pink-100 rounded-2xl shadow-xl p-6 border border-red-200 relative overflow-hidden hover:shadow-2xl transition-all duration-300 group transform hover:scale-105">
            <div className="relative z-10">
              ...
              <p className="text-3xl font-bold text-red-900 mb-1">
                {formatCurrency(
                  clientProfitData.reduce((sum, c) => sum + c.creditAmount, 0)
                )}
              </p>
        ...
        {viewMode === "cards" ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredData.map((client, index) => {
              ...
              const paymentSpeed = getPaymentSpeedBadge(
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <ChevronRight
                      className={`h-5 w-5 text-gray-400 transition-transform ${selectedClient === client.id ? "rotate-90" : ""}`}
                    />
                  </div>
                  ...
                  <div className="mb-4">
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-3 shadow-inner">
                      <div
                        ...
                        style={{
                          width: `${Math.min((client.totalRevenue / summaryMetrics.topClientRevenue) * 100, 100)}%`,
                        }}
                      ></div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <span
                      className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${paymentSpeed.color}`}
                    >
                      ...
                      {selectedClientData.revenueByMonth.length > 0 && (
                        <div>
                          ...
                          <div className="h-32">
                            <ResponsiveContainer width="100%" height="100%">
                              <AreaChart
                                data={selectedClientData.revenueByMonth}
                              >
                                <defs>
                                  <linearGradient
                                    id="revenueGradient"
                          ...
                          <div className="space-y-1 text-sm">
                            <div className="flex justify-between">
                              ...
                              <span className="font-medium">
                                {formatCurrency(client.grossRevenue)}
                              </span>
                            </div>
                      ...
                      <div className="grid grid-cols-3 gap-4">
                        <div className="text-center p-3 bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl border border-green-200">
                          <div className="text-lg font-bold text-green-700">
                            {selectedClientData.paidInvoices}
                          </div>
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right">
                          ...
                          <div className="text-xs text-gray-500">
                            {formatCurrency(client.averageInvoiceValue)} avg
                          </div>
                        </td>
